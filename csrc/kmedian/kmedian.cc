...
public:
	...
	float CC(int i, int j)
	{
		...
		return floor(m - i + 1.0) * mu - sum_below +
			sum_above - ceil(j - m - 1.0) * mu;
	}
	...
	std::vector<float> optimalKMedian(int numClusters)
	{
		...
		std::copy(cumulativeSum.begin(), cumulativeSum.end(),
			std::ostream_iterator<float>(std::cout, " "));
		std::cout << std::endl;
		...
		for (int i=1; i<numClusters+1; i++)
		{
			...
			std::copy(T[i].begin(), T[i].end(), std::ostream_iterator<int>(std::cout, " "));
			std::cout << std::endl;
			std::copy(D[i].begin(), D[i].end(), std::ostream_iterator<int>(std::cout, " "));
			std::cout << std::endl;
		}
		...
		for (int i=numClusters; i>0; i--)
		{
			...
			std::cout << "New: " << newClusterRange << " old: " << currentClusteringRange << std::endl;
...
bool test()
{
	std::vector<float> test_case = {22, 36, 35, 22, 22, 23, 37, 22, 35, 47};
	std::vector<float> expected = {22, 34, 47.5};

	KCluster test(test_case);

	std::vector<float> observed = test.optimalKMedian(3);

	std::cout << "Expected: ";
	std::copy(expected.begin(), expected.end(), std::ostream_iterator<float>(std::cout, " "));
	std::cout << "\nObserved: ";
	std::copy(observed.begin(), observed.end(), std::ostream_iterator<float>(std::cout, " "));
	std::cout << std::endl;
